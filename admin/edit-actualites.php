...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $type_actualite_id = $_POST["type_actualite_id"];
  $erreurs = [];
  ...
  if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
      ...
      $sql = "UPDATE `actualites` SET `titre`='$titre',`description`='$description',`type_actualite_id`='$type_actualite_id', `filename`='$filename' WHERE id='$id'";
      $update_actualite = $bdd->query($sql);
      //var_dump($update_actualite); die();
...
$req = $bdd->prepare("SELECT * FROM `type_actualites` WHERE 1");
$req->execute();

$types = $bdd->prepare("SELECT * FROM `type_actualites` WHERE 1");
$types->execute();
...
<body class="bc1">
  ...
  <div class="container-fluid">
          ...
          <div class="bc8"><h3>Modifier</h3></div>

      <div class="bc2">
            <br>
          <form method="post" enctype="multipart/form-data">
              <div class="input-group">
                <div class="bc3"><label for="titre"><b>Titre</b></label></div>
                <input type="text" class="bc9" id="titre" name="titre" value="<?php echo $data['titre']; ?>" required>
              </div>
                <br>
              <div class="input-group">
                <div class="bc4"><label for="description"><b>Description</b></label></div>
              ...
              <br>
              <div class="input-group">
                <div class="bc5"><label for="filename"><b>Filename</b></label></div>
                <input type="file" id="filename" name="file" required>
              </div>
                  <br>
              <div class="input-group">
                    <div class="bc6">
                        <select required name="type_actualite_id" id="type_actualite_id">
                            <option value="<?php echo $data['type_actualite_id']; ?>"></option>
                            <?php
                            $i = 0;
                            while ($row = $req->fetch()) {
                                $i++;
              ...
              <div class="input-group">
                <div class="bc7"><button type="submit" name="submit" class="btn btn-secondary mb-4"><b>Ajouter</b></button></div>
              </div>
          </form>
        </div>
  </div>
